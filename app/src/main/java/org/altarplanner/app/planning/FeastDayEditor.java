...
public class FeastDayEditor {
  ...
  private void initialize() {
    removeButton
        .disableProperty()
        .bind(feastDayListView.getSelectionModel().selectedItemProperty().isNull());
    datePicker
        .disableProperty()
        .bind(feastDayListView.getSelectionModel().selectedItemProperty().isNull());
        ...
        .addListener(
            (observable, oldValue, newValue) -> {
              if (applyDatePickerChanges) {
                feastDayListView
                    .getItems()
                    .remove(feastDayListView.getSelectionModel().getSelectedIndex());
  ...
  @FXML
  private void addFeastDay() {
    addAndSelect(LocalDate.now().plusMonths(1));
  }

  @FXML
  private void removeFeastDay() {
