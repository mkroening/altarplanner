...
public class PoiIO {
    ...
    public static void exportSchedule(Schedule schedule, File file, int columns) throws IOException {
        ...
        schedule.getPlanningMasses().forEach(mass -> {
    ...
    public static void exportScheduleOverview(Schedule schedule, File file) throws IOException {
        ...
        schedule.getDateMassesMap().forEach((date, masses) -> {
            final int firstMassColumn = columnOffset + schedule.getPlanningMasses().indexOf(masses.get(0));
        ...
        IntStream.range(0, schedule.getPlanningMasses().size())
                .forEach(value -> {
                    PlanningMass mass = schedule.getPlanningMasses().get(value);
        ...
        IntStream.range(0, schedule.getServers().size())
                .forEach(serverIndex -> {
                    ...
                    IntStream.range(0, schedule.getPlanningMasses().size())
                            .forEach(massIndex -> schedule.getPlanningMasses().get(massIndex).getServices().parallelStream()
