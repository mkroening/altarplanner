...
class ScoreConstraintTests {
        ...
        .mapToObj(
            value -> {
              ...
              planningMassTemplate.setDateTime(
                  LocalDateTime.of(
                      subsequentMasses ? LocalDate.now().plusDays(value) : LocalDate.now(),
                      LocalTime.of(11, 0)));
              planningMassTemplate.setServiceTypeCounts(
                  Map.of(config.getServiceTypes().get(subsequentServiceTypes ? value : 0), 1));
  ...
  void oneServicePerDay() {
    ...
    List<PlanningMassTemplate> planningMassTemplates =
        generatePlanningMassTemplates(config, false, false);
    ...
    IntStream.range(0, massCount)
        .forEach(
            value -> {
              ...
              int expectedWeight =
                  -value * (value + 1) / 2; // equal to -IntStream.range(0, value + 1).sum()
  ...
  void notEnoughExp() {
    ...
    List<PlanningMassTemplate> planningMassTemplates =
        generatePlanningMassTemplates(config, true, true);
        ...
        .forEach(
            value -> {
              ...
              schedule
                  .getServices()
                  .get(value)
                  .setServer(schedule.getServers().get(massCount - 1 - value));
              scoreVerifier.assertHardWeight(
                  constraintName, massCount - 1 - value < value ? -1 : 0, schedule);
  ...
  void dateOffRequest() {
    ...
    config
        ...
        .get(0)
        .getAbsences()
        .add(LocalDateRange.ofClosed(LocalDate.now(), LocalDate.now().plusDays(massCount / 2)));
    config.getServiceTypes().add(new ServiceType());

    List<PlanningMassTemplate> planningMassTemplates =
        generatePlanningMassTemplates(config, true, false);
    ...
    IntStream.range(0, massCount)
        .forEach(
            value -> {
              schedule.getServices().get(value).setServer(schedule.getServers().get(0));
              scoreVerifier.assertHardWeight(
                  constraintName, value <= massCount / 2 ? -1 : 0, schedule);
              schedule.getServices().get(value).setServer(null);
            });
  }
    ...
    config
        ...
        .addAll(
            IntStream.range(0, massCount / 2)
                .mapToObj(
                    value ->
                        LocalDateTime.of(LocalDate.now().plusDays(2 * value), LocalTime.of(11, 0)))
                .collect(Collectors.toUnmodifiableList()));
    config.getServiceTypes().add(new ServiceType());
    ...
    IntStream.range(0, massCount)
        .forEach(
            value -> {
              ...
              scoreVerifier.assertSoftWeight(
                  constraintName, -other * other - assignments * assignments, schedule);
            });
  }
}
